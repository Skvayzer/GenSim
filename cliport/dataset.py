...
class MyCustomDataset(Dataset):
    ...
    def __init__(self, path, task_name, cfg, n_demos=0, augment=False):
        """A simple RGB-D image dataset."""
        self._path = path
        ...
        for fname in sorted(os.listdir(self._path)):
            if f'{episode_id:04d}' in fname:
                ...
                for i in range(len(colors)):
                    ...
                    if len(info['lang_goal']) == 0:
                        break
                    reconstructed_episode.append((obs, single_action, rewards[i], info))
                # print('\n\n\n episode', reconstructed_episode, '\n\n\n')
                return reconstructed_episode, seed
            
        print(f'{episode_id:04d} not in ', self._path)
    ...
    def process_goal(self, goal, perturb_params):
        ...
        # try:
        if perturb_params is not None and len(perturb_params) > 1:
            img = utils.apply_perturbation(np.array(img), perturb_params)
